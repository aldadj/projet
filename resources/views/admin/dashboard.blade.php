            ...
            @forelse($articles as $article)
                <div class="group bg-gray-900 rounded-xl border border-gray-700 overflow-hidden hover:border-gray-500 transition-all shadow-lg hover:shadow-2xl flex flex-col">
                    <div class="h-40 overflow-hidden relative">
                         @if($article->image)
                            <img src="{{ asset($article->image) }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                         @else
            ...
            <table class="w-full text-left text-gray-300">
                ...
                <tbody class="divide-y divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-700/50 transition-colors">
                        <td class="p-4 font-medium text-white">{{ $msg->name }}</td>
                        <td class="p-4">{{ $msg->subject }}</td>
                        <td class="p-4 text-sm text-gray-400">{{ $msg->created_at->format('d/m H:i') }}</td>
                        <td class="p-4 text-right">
                            <a href="{{ route('admin.message.show', $msg->id) }}" class="text-blue-400 hover:text-white font-bold text-sm transition-colors">Lire</a>
                        </td>
                    </tr>
                    @empty
                    <tr>
