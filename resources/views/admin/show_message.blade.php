...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-8">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.dashboard') }}" class="p-2 bg-slate-800 rounded-full text-slate-400 hover:text-white hover:bg-slate-700 transition">
        ...
        <div class="flex gap-3">
            <a href="mailto:{{ $message->email }}?subject=Re: {{ $message->subject }}" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg font-bold text-sm transition shadow-lg shadow-blue-900/20">
            ...
            <form action="{{ route('admin.message.delete', $message->id) }}" method="POST" onsubmit="return confirm('Supprimer ce message définitivement ?');">
                @csrf @method('DELETE')
        ...
        <div class="p-8 border-b border-slate-700 bg-slate-900/30 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <div>
                        <h2 class="text-xl font-bold text-white leading-none">{{ $message->name }}</h2>
                        <a href="mailto:{{ $message->email }}" class="text-blue-400 hover:text-blue-300 text-sm font-medium transition-colors">
                            {{ $message->email }}
                        </a>
                    </div>
                </div>
