...
<div class="max-w-5xl mx-auto px-4 py-8">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        <div class="flex items-center gap-6">
            <a href="{{ route('admin.dashboard') }}" class="group flex items-center justify-center w-12 h-12 bg-white border-2 border-slate-200 rounded-full text-slate-600 hover:border-[#bb1919] hover:text-[#bb1919] transition-all shadow-sm">
        ...
        <div class="flex gap-4 w-full md:w-auto">
            <a href="mailto:{{ $message->email }}?subject=Re: {{ $message->subject }}" class="flex-1 md:flex-none flex items-center justify-center gap-3 bg-[#212121] hover:bg-[#bb1919] text-white px-6 py-3 font-black text-xs uppercase tracking-widest transition-all shadow-lg">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" />
                </svg>
                Répondre
            </a>
            <form action="{{ route('admin.message.delete', $message->id) }}" method="POST" onsubmit="return confirm('Supprimer définitivement ?');" class="flex-1 md:flex-none">
                @csrf @method('DELETE')
    ...
    <div class="bg-white border-t-4 border-[#bb1919] shadow-2xl overflow-hidden">
        ...
        <div class="p-8 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row justify-between items-start gap-8">
            <div class="flex items-start gap-5">
                ...
                <div>
                    <h2 class="text-2xl font-black text-slate-900 leading-none uppercase tracking-tighter">{{ $message->name }}</h2>
                    <div class="mt-2 flex flex-col gap-1">
                        <a href="mailto:{{ $message->email }}" class="text-[#bb1919] hover:underline text-sm font-bold tracking-tight italic">
                            {{ $message->email }}
                        </a>
                        <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                            ...
                            Reçu le {{ $message->created_at->format('d/m/Y') }} à {{ $message->created_at->format('H:i') }}
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="p-8 md:p-12">
            ...
            <div class="relative">
                {{-- Guillaumet de décoration style News --}}
